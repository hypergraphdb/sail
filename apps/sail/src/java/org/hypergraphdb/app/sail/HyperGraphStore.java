...
public class HyperGraphStore extends SailBase {
  ...
  /**
   * Creates a new HyperGraph RDF Store using the provided database connection.
   *
   * @param location location of a DataSource
   */
  ...
  private void createIndex(HGTypeSystem ts, Class c, String alias, String field) {
    ...
    typeH = graph.getPersistentHandle(typeHandle);
       ...
       if (check == null)
         ...
         throw new RuntimeException("Alias already in use: "+alias+", please change "+alias+" prefix.");
    graph.getIndexManager().register(new ByPartIndexer(typeH, new String[]{field}));
  ...
  public HGPersistentHandle lookupNameSpacePrefix(String prefix) {
    HGHandle typeHandle = graph.getTypeSystem().getTypeHandle("sail_NamesSpace");
    ByPartIndexer byProperty = new ByPartIndexer(typeHandle, new String[]{"prefix"});
  ...
  public HGPersistentHandle lookupStatementSubject(String subject) {
    HGHandle typeHandle = graph.getTypeSystem().getTypeHandle("sail_ContextStatement");
    ByPartIndexer byProperty = new ByPartIndexer(typeHandle, new String[]{"subject"});
    HGIndex<Object, HGPersistentHandle> index = graph.getIndexManager().getIndex(byProperty);
    return (HGPersistentHandle) index.findFirst(subject);

  }

  public HGPersistentHandle lookupContext(String context) {
    HGHandle typeHandle = graph.getTypeSystem().getTypeHandle("sail_Context");
    ByPartIndexer byProperty = new ByPartIndexer(typeHandle, new String[]{"context"});
    HGIndex<Object, HGPersistentHandle> index = graph.getIndexManager().getIndex(byProperty);
    return (HGPersistentHandle) index.findFirst(context);

  }

  public HGPersistentHandle lookupValue(String val) {
    HGHandle typeHandle = graph.getTypeSystem().getTypeHandle("sail_Value");
    ByPartIndexer byProperty = new ByPartIndexer(typeHandle, new String[]{"val"});
    HGIndex<Object, HGPersistentHandle> index = graph.getIndexManager().getIndex(byProperty);
