 ...
 */
public abstract class RDFStoreTest2 extends TestCase {
	...
	public RDFStoreTest2(String name) {
		super(name);
	}
  ...
  public void dumpDB() {
   ...
   //   dumpDB();
			...
			public void run() {
				...
				try {
					...
					while (result.hasNext()) {
                     ...
                     // System.out.println("MT:S:"+st.getSubject()+"\tP:"+st.getPredicate()+"\tO:"+st.getObject());
		...
		throws Exception
	{
		...
		assertEquals(context1.stringValue(), first(con.getContextIDs()).stringValue());
		...
		assertEquals(1, countElements(con.getContextIDs()));
		assertEquals(context2.stringValue(), first(con.getContextIDs()).stringValue());
		con.commit();
	}
